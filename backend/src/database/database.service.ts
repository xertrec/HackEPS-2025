...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  getAllNeighborhoods(): Promise<Neighborhood[]> {
    return new Promise((resolve, reject) => {
      this.db.all('SELECT * FROM neighborhoods ORDER BY name', (err, rows: Neighborhood[]) => {
        if (err) {
          reject(err);
        } else {
          resolve(rows);
  ...
  updateNeighborhoodScores(
    ...
    const totalScore = Math.round((nightlifeScore + shopsScore + leisureScore) / 3);
  ...
  getNeighborhoodsNearby(lat: number, lon: number, radiusKm: number = 5): Promise<Neighborhood[]> {
